...
const Navbar = ({ customGradient }) => {
  ...
  const navLinks = [
    ...
    { title: 'ChatBot', path: '/chatbot' }  // Changed from Gallery
  ];
  ...
  className={`fixed top-0 w-full z-50 ${customGradient || 'bg-gradient-to-r from-lime-400 via-green-500 to-green-800'}`}
>
        <div className="flex items-center justify-between h-16 px-6 sm:px-8">
          <motion.div 
            ...
            className="relative"
          >
            <Link to="/" className="text-2xl font-bold bg-gradient-to-r from-blue-800 to-black bg-clip-text text-transparent flex items-center gap-2">
              <img 
                src="https://icon-library.com/images/plant-icon-png/plant-icon-png-0.jpg" 
                alt="FarmWise Logo" 
                className="h-8 w-8 object-contain filter brightness-0" // brightness-0 makes it black
              />
              FarmWise
            </Link>
          </motion.div>
          ...
          <div className="hidden sm:flex items-center space-x-12">
            ...
            >
              <Link 
                to='/contact' 
                className="bg-gradient-to-r from-blue-500 to-violet-500 text-white px-6 py-2 rounded-full 
